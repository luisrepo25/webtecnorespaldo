...
class CarreraService
{
    ...
    public function registrarCarrera(Request $request)
    {
        ...
        $request->validate($this->reglasValidacion($esCurso), $this->mensajesValidacion());

        Carrera::create($this->datosCarrera($request, $esCurso) + ['activo' => true]);
    ...
    public function actualizarCarrera(Request $request, int $id)
    {
        ...
        $request->validate($this->reglasValidacion($esCurso, $id), $this->mensajesValidacion());

        $carrera->update($this->datosCarrera($request, $esCurso));
    ...
    // ── Deriva modalidad/duracion_unidad según tipo (curso_libre vs carrera) ──
    private function datosCarrera(Request $request, bool $esCurso): array
    {
        return [
    ...
    private function reglasValidacion(bool $esCurso, ?int $ignoreId = null): array
    {
        return [
            'codigo'                 => ['required','string','max:20', $ignoreId
                ? Rule::unique('carreras', 'codigo')->ignore($ignoreId, 'id_carrera')
                : 'unique:carreras,codigo', 'regex:/^[a-zA-Z0-9\-_]+$/'],
